-- =============================================================================
-- 0050_comment_edit_delete.sql — author edit/delete of their own comment
-- =============================================================================
-- Today a comment's root note has no edit path at all, and deleting it goes
-- through delete_review_thread which is workspace-OWNER-only — so a reviewer
-- (guest or member) who made a mistake is stuck with their own comment. This
-- lets the AUTHOR edit or delete their own comment, but only while it is
-- "untouched by others": open, no replies at all, and never sent to the agent.
-- Once anyone else engages (a reply, a send-to-agent, or a resolve), the
-- original is frozen; the author continues via replies (reply-delete is already
-- author-only, unchanged). The workspace owner keeps their delete override.
--
-- Single "untouched" gate for BOTH edit and delete (one rule to reason about),
-- and it also forecloses a security edge: editing a comment's reference images
-- after a send could otherwise slip an unreviewed image past the agent-confirm
-- step — but edit is impossible after a send, so it can't.
--
-- Three changes:
--   1. list_review_comments: add is_own (caller authored it) + is_sent (a
--      comment_queue row exists) so the OVERLAY can show/hide edit+delete and
--      explain a lock without a fail-after-click. Return signature changes, so
--      DROP + CREATE. Additive to the deployed overlay (it ignores new columns
--      until its bundle is redeployed). The DASHBOARD already has status /
--      sendStatus / latestReplyAt and computes the gate client-side.
--   2. delete_review_thread: broaden authorization to OWNER OR (AUTHOR AND
--      untouched). Same name/signature. Owner check now uses the resolved member
--      id so an owner reviewing via the overlay session works too.
--   3. edit_review_comment (new): author-only + untouched; updates note and
--      context.referenceImages (ref-shape pinned to the preview, as 0048/0049).
--
-- REAL-ENV GATE (rolled-back txn before push): author can edit/delete an
-- untouched own comment; a comment with a reply / a queue row / resolved is
-- rejected; a non-author member is rejected by edit but the OWNER can still
-- delete; a cross-preview image ref is rejected; get_advisors(security) clean.
-- =============================================================================

-- ---------------------------------------------------------------------------
-- 1. list_review_comments — add is_own + is_sent (return type change → drop).
-- ---------------------------------------------------------------------------
...
begin
  ...
  -- Canonical caller identity (member uid via session, else the anon uid) so
  -- is_own matches the participant.user_id the write RPCs check.
  ...
  select
    ...
    exists (select 1 from public.comment_queue q where q.comment_id = c.id) as is_sent
...
-- ---------------------------------------------------------------------------
-- 2. delete_review_thread — OWNER OR (AUTHOR AND untouched).
-- ---------------------------------------------------------------------------
...
begin
  ...
  select c.preview_id, c.status, (pt.user_id = v_caller)
    into v_preview, v_status, v_is_author
    from public.comments c
    join public.participants pt on pt.id = c.author_participant
  ...
  -- Untouched by others: open, no replies at all, never sent to the agent.
...
-- ---------------------------------------------------------------------------
-- 3. edit_review_comment (new) — AUTHOR AND untouched; note + reference images.
-- ---------------------------------------------------------------------------
